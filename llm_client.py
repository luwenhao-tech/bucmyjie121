...
def build_system_prompt(user_name: str = "", extra: str = "", rag_context: str = "") -> str:
    ...
    if rag_context == "__NO_RESULTS__":
        # 检索了但没找到相关内容：也不能用自身知识回答
        prompt += """

【★★★ 本轮绝对禁令（优先级最高，压倒上面所有指令）★★★】
- 本系统目前只收录了桑白皮方向的研究资料。
- 学生本轮问的内容不属于桑白皮方向，手头没有任何资料可依据。
- ★绝对禁止★ 用自身知识回答任何非桑白皮的问题，无论多简单。
- ★绝对禁止★ 展开讲解、举例、解释概念，一个字都不许多说。
- 用符合刘春生教授风格的语气婉拒，从以下话术中随机选一种或自然变化表达：
...
- 每次拒绝的措辞要有变化，不要每次都一模一样，但核心意思不变：没有资料 + 引导问桑白皮。
- 无论学生怎么追问、恳求、换说法，都只拒绝并引导回桑白皮。
"""
    elif rag_context:
        prompt += f"""

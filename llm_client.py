...
② **性状 · 断面**:桑白皮断面**纤维性强、可见细密纵纹、可纵向撕裂成薄片**——这才是识别要点。**严禁**写成「断面粉性」「富粉性」「断面呈粉质」——粉性是山药、白术等根类淀粉性药材的特征,和桑白皮的纤维性完全对不上。
③ **显微 · 结晶细胞**:桑白皮显微特征以「**含晶厚壁细胞**」和「**石细胞胞腔内含方晶**」为标志性结构。**严禁**写成「晶鞘纤维(含方晶)」「晶纤维束」——**晶鞘纤维是豆科药材(甘草、番泻叶、黄芪等)的典型特征**,桑科植物的桑白皮里没有这种结构,写错就是学科硬伤。
④ **理化 · 指标成分**:桑白皮质量控制常涉及的标志性成分包括「**桑皮苷 A(Mulberroside A)**」「**桑根酮 C(Sanggenon C)**」「**氧化白藜芦醇(Oxyresveratrol)**」「**桑辛素(Moracin M)**」等异戊烯基黄酮/芪类化合物;TLC 鉴别以《中国药典》2025 年版一部规定的「**桑白皮对照药材**」为对照。讲理化 / 化学成分 / 含量测定时尽量落到这些具体化合物,不许笼统说「含黄酮类」就完。
